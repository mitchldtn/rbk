...
impl Grid {
    ...
    /// Write the last `n` scrollback lines plus all current screen rows.
    /// Unlike `write_contents`, this is not bounded by the visible window.
    pub fn write_scrollback_and_screen(
        &self,
        contents: &mut String,
        scrollback_lines: usize,
    ) {
        let sb_len = self.scrollback.len();
        let skip = sb_len.saturating_sub(scrollback_lines);
        let mut wrapping = false;
        for row in self.scrollback.iter().skip(skip).chain(self.rows.iter())
        {
            row.write_contents(contents, 0, self.size.cols, wrapping);
            if !row.wrapped() {
                contents.push('\n');
