...
impl Screen {
    ...
    /// Returns the last `scrollback_lines` of scrollback plus all current
    /// screen rows as plain text.  Unlike `contents()`, this is not bounded
    /// by the visible window size.
    #[must_use]
    pub fn scrollback_and_screen(&self, scrollback_lines: usize) -> String {
        let mut contents = String::new();
        self.grid()
            .write_scrollback_and_screen(&mut contents, scrollback_lines);
        contents
    }
